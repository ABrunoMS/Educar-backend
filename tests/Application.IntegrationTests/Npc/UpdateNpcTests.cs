...
public class UpdateNpcTests : TestBase
{
    ...
    public async Task GivenValidRequest_ShouldUpdateNpc()
    {
        ...
        var updateGameCommand = new CreateGameCommand(
            "Updated Game",
            "Updated Game Description",
            "Updated Game Lore",
            "Updated Game Purpose");
        var updatedGameResponse = await SendAsync(updateGameCommand);
        ...
        var updateCommand = new UpdateNpcCommand
        {
            ...
            GameIds = new List<Guid> { updatedGameResponse.Id }
        };
        ...
        if (Context.Npcs != null)
        {
            ...
            Assert.That(updatedNpc.GameNpcs, Has.Count.EqualTo(1));
            Assert.That(updatedNpc.GameNpcs.First().Game.Name, Is.EqualTo("Updated Game"));
        }
    }
    ...
    [Test]
    public async Task ShouldUpdateNpcWithGames()
    {
        const string originalName = "Original Name";
        const string originalLore = "Original Lore";
        const NpcType originalNpcType = NpcType.Boss;
