...
[TestFixture]
public class MediaUpdatedEventHandlerTests : TestBase
{
    private Mock<IApplicationDbContext> _contextMock;
    private MediaUpdatedEventHandler _handler;

    [SetUp]
    public void SetUp()
    {
        _contextMock = new Mock<IApplicationDbContext>();
        _handler = new MediaUpdatedEventHandler(_contextMock.Object);
    }
    ...
    public async Task GivenValidEvent_ShouldCreateMediaLogAndSaveChanges()
    {
        ...
        var previousState = new JsonObject { ["state"] = "old state" };
        var notification = new MediaUpdatedEvent(media, accountId, previousState);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(createdLog.PreviousState, Is.Not.Null);
            Assert.That(createdLog.PreviousState?.ToJsonString(), Is.EqualTo(previousState.ToJsonString()));
            Assert.That(createdLog.Action, Is.EqualTo(AuditableAction.Update));
        });
    }
